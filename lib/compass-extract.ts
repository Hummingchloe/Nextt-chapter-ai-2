  ...
  input_schema: {
    ...
    properties: {
      sentence: {
        type: "string",
        description:
          "한 문장. 가치 나열 금지. 본질을 포착. 예: '안정보다 자율을 택하되, 아는 사람부터 1:1로 돕는 사람'. '~하는 사람' 형태 권장.",
      },
    },
    required: ["sentence"],
  },
...
export async function synthesizeEssence(
  ...
  const system = [
    "당신은 한 사람의 의사결정 구슬들을 읽고, 그 사람의 '일의 방향'을 한 문장으로 압축합니다.",
    "가치를 나열하지 말고, 본질을 포착하는 단 하나의 문장으로 쓰세요(높은 추상도).",
    "단정·과장 금지. 잠정적이되 선명하게. 한국어. '~하는 사람' 형태를 권장.",
  ].join("\n");
