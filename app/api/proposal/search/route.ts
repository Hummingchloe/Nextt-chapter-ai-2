...
import { searchProposalContentWithAI } from "@/lib/proposal-ai";
...
export async function POST(req: Request) {
  ...
  if (ontology.compass.alignment < 50) {
    return NextResponse.json({ error: "alignment_below_threshold" }, { status: 409 });
  }

  const result = await searchProposalContentWithAI(ontology);
